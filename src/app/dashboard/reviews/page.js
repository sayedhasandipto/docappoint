    ...
    useEffect(() => {
        const fetchData = async () => {
            if (session?.user?.email) {
                try {
                    // Fetch bookings
                    const resBookings = await fetch(`http://localhost:8000/appointments?email=${session.user.email}`);
                    ...
                    // Fetch reviews
                    const resReviews = await fetch(`http://localhost:8000/reviews?email=${session.user.email}`);
